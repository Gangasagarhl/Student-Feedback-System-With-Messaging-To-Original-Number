...
function sending_messge($message,$mobile_number)
{
	...
	( $curl, array
		(
			...
			(
				"authorization:gHJHpdb7F2Jt018dm3H5wfC1k6eTj3IlfGPe64ntTXS6rnMvLidSWPQ4NDws",
				"accept: */*",
...
function sum_average_feedback_with_comments($conn,$admin,$user,$sem)
{
        ...
        {   
            ...
            {   
                ...
                $arr[$m]['s1']=$rows2['s1'];    $arr[$m]['a1']=(int)$rows2['a1'];
                $arr[$m]['s2']=$rows2['s2'];    $arr[$m]['a2']=(int)$rows2['a2'];
                $arr[$m]['s3']=$rows2['s3'];    $arr[$m]['a3']=(int)$rows2['a3'];
                $arr[$m]['s4']=$rows2['s4'];    $arr[$m]['a4']=(int)$rows2['a4'];
                $arr[$m]['s5']=$rows2['s5'];    $arr[$m]['a5']=(int)$rows2['a5'];
...
function preparing_and_send_message($conn,$admin,$time)
{
    ...
    for($i=0;$i<count($all_employee_for_an_admin[0]);$i+=1)
    {   
